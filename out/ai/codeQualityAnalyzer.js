...
class CodeQualityAnalyzer {
    constructor() {
        this.aiAnalyzer = new variableAnalyzer_1.AIVariableAnalyzer();
    }
    /**
     ...
     */
    analyzeCodeQuality(variables, fileContent) {
        // Calculate individual variable scores
        const variableScores = variables.map(variable => this.aiAnalyzer.calculateVariableQualityScore(variable, variables));
        ...
        const overallScore = this.calculateOverallScore(variableScores, fileComplexity);
        // Generate general suggestions based on the analysis
        const generalSuggestions = this.generateGeneralSuggestions(variableScores, fileComplexity);
        return {
            overallScore,
            variableScores,
            fileComplexity,
     ...
     */
    calculateFileComplexity(variables, fileContent) {
        ...
        const globalVariablePercentage = totalVariables > 0 ? (globalVariables.length / totalVariables) * 100 : 0;
        // Calculate variable reuse factor (average references per variable)
        const totalReferences = variables.reduce((sum, v) => sum + v.references.length, 0);
        const variableReuseFactor = totalVariables > 0 ? totalReferences / totalVariables : 0;
        // Estimate cyclomatic complexity based on conditionals and loops
        const conditionalMatches = (fileContent.match(/if|else|switch|case|for|while|do/g) || []).length;
        ...
        const maintainabilityIndex = Math.max(0, Math.min(100, 100 -
            0.2 * cycloComplexity -
            0.1 * linesOfCode -
     ...
     */
    calculateOverallScore(variableScores, fileComplexity) {
        // If no variables, return a neutral score
        if (variableScores.length === 0) {
            return 50;
        }
        // Average variable quality score (40% weight)
        const avgVariableScore = variableScores.reduce((sum, score) => sum + score.overallScore, 0) /
        ...
        const overallScore = Math.round(avgVariableScore * 0.4 +
            globalVarScore * 0.1 +
     ...
     */
    generateGeneralSuggestions(variableScores, fileComplexity) {
        const suggestions = [];
        // Analyze naming consistency
        const lowNamingScores = variableScores.filter(score => score.namingScore < 70);
        if (lowNamingScores.length > 3 ||
            (variableScores.length > 10 &&
                lowNamingScores.length / variableScores.length > 0.3)) {
            suggestions.push('Consider adopting a consistent naming convention throughout the file');
        }
        // Analyze global variable usage
        if (fileComplexity.globalVariablePercentage > 30) {
            suggestions.push('Reduce the number of global variables to improve maintainability');
        }
        // Analyze variable reuse
        if (fileComplexity.variableReuseFactor < 1) {
            suggestions.push('Many variables are declared but rarely used. Consider refactoring to reduce unused variables');
        }
        else if (fileComplexity.variableReuseFactor > 10) {
            suggestions.push('Some variables are used extensively. Consider breaking down complex functions or introducing additional variables for clarity');
        }
        // Analyze complexity
        if (fileComplexity.cycloComplexity > 15) {
            suggestions.push('Code has high cyclomatic complexity. Consider refactoring complex conditional logic into smaller, more manageable functions');
        }
        // Analyze maintainability
        if (fileComplexity.maintainabilityIndex < 50) {
            suggestions.push('Code maintainability is low. Consider refactoring for improved readability and reduced complexity');
        }
        // If code is generally good
        if (suggestions.length === 0 && fileComplexity.maintainabilityIndex > 80) {
            suggestions.push('Code quality is good. Maintain consistent practices as the codebase grows');
        }
        return suggestions;
    }
    /**
    ...
    generateQualityReport(metrics) {
        ...
        <tr>
          <td>${score.name}</td>
          <td style="text-align: center; background-color: ${getScoreColor(score.namingScore)}">${score.namingScore}</td>
          <td style="text-align: center; background-color: ${getScoreColor(score.usageScore)}">${score.usageScore}</td>
          <td style="text-align: center; background-color: ${getScoreColor(score.scopeScore)}">${score.scopeScore}</td>
          <td style="text-align: center; background-color: ${getScoreColor(score.overallScore)}">${score.overallScore}</td>
      ...
      <body>
        ...
        <h2>Overall Score</h2>
        <div class="score-badge" style="background-color: ${getScoreColor(metrics.overallScore)}">
        ...
        <div class="metrics-grid">
          <div class="metric-card">
            ...
            <div class="metric-value">${metrics.fileComplexity.totalVariables}</div>
          </div>
          <div class="metric-card">
            <div class="metric-title">Unique Variable Types</div>
            <div class="metric-value">${metrics.fileComplexity.uniqueVariableTypes}</div>
          </div>
          <div class="metric-card">
            <div class="metric-title">Global Variable %</div>
            <div class="metric-value">${metrics.fileComplexity.globalVariablePercentage.toFixed(1)}%</div>
          </div>
          <div class="metric-card">
            <div class="metric-title">Variable Reuse Factor</div>
            <div class="metric-value">${metrics.fileComplexity.variableReuseFactor.toFixed(2)}</div>
          </div>
          <div class="metric-card">
            <div class="metric-title">Cyclomatic Complexity</div>
            <div class="metric-value">${metrics.fileComplexity.cycloComplexity}</div>
          </div>
          <div class="metric-card">
            <div class="metric-title">Maintainability Index</div>
            <div class="metric-value" style="color: ${getScoreColor(metrics.fileComplexity.maintainabilityIndex)}">
