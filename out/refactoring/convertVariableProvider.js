...
exports.ConvertVariableProvider = void 0;
const vscode = __importStar(require("vscode"));
class ConvertVariableProvider {
    /**
     * Converts a variable declaration from one type to another (var/let/const)
     * @param context The extension context
     */
    static async convertVariable(context) {
        try {
            ...
            const varMatch = lineText.match(/\b(var|let|const)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/);
            if (!varMatch) {
                vscode.window.showErrorMessage('No variable declaration found on this line');
                return;
            }
            const currentType = varMatch[1];
            const variableName = varMatch[2];
            // Ask for new variable type
            const newType = await vscode.window.showQuickPick(['var', 'let', 'const'].filter(type => type !== currentType), {
            ...
            const keywordRange = new vscode.Range(new vscode.Position(position.line, keywordIndex), new vscode.Position(position.line, keywordIndex + currentType.length));
            // Replace the keyword
            edit.replace(editor.document.uri, keywordRange, newType);
            // Apply the edit
            const success = await vscode.workspace.applyEdit(edit);
            if (success) {
                vscode.window.showInformationMessage(`Converted ${currentType} ${variableName} to ${newType}`);
            }
            else {
     ...
     * @param context The extension context
     */
    static async smartConvertVariable(context) {
        try {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showErrorMessage('No active editor found');
            ...
            const suggestion = this.analyzeVariableUsage(editor.document, variableName, currentType);
            ...
            if (newType === currentType) {
                vscode.window.showInformationMessage(`${currentType} ${variableName} is already correctly declared`);
                return;
            }
            // Create the workspace edit
            const edit = new vscode.WorkspaceEdit();
     ...
     */
    static analyzeVariableUsage(document, variableName, currentType) {
        const text = document.getText();
        // Check if the variable is reassigned
        const reassignmentRegex = new RegExp(`\\b${this.escapeRegExp(variableName)}\\s*=`, 'g');
        ...
        const declarationRegex = new RegExp(`\\b(var|let|const)\\s+${this.escapeRegExp(variableName)}\\b`, 'g');
...
exports.ConvertVariableProvider = ConvertVariableProvider;
//# sourceMappingURL=convertVariableProvider.js.map