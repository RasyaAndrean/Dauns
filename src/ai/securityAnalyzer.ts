...
export class SecurityAnalyzer {
  ...
  private static readonly UNSAFE_FUNCTIONS = [
    /eval\s*\(/i,
    /setTimeout\s*\(\s*['"`]/,
    /setInterval\s*\(\s*['"`]/,
    /Function\s*\(\s*['"`]/,
   ...
   */
  public analyzeSecurityVulnerabilities(
  ...
  private checkSensitiveDataExposure(
    ...
    const isSensitive = SecurityAnalyzer.SENSITIVE_PATTERNS.some(pattern =>
      ...
      for (const ref of variable.references) {
        ...
        if (isExposed) {
          vulnerabilities.push({
            ...
            description: `Sensitive data in variable '${
              variable.name
            }' is potentially exposed through ${
              refLine.includes('console.log')
                ? 'logging'
                : refLine.includes('localStorage') ||
                  refLine.includes('sessionStorage')
            ...
            recommendation:
              'Avoid exposing sensitive data through logs, browser storage, or client-side output. Consider using secure storage methods or encryption.',
            code: refLine.trim(),
            cwe: 'CWE-200',
          });
        }
  ...
  private checkHardcodedCredentials(
    ...
    const isCredential = SecurityAnalyzer.SENSITIVE_PATTERNS.some(pattern =>
      pattern.test(variable.name)
    );
    ...
    if (isCredential && variable.value) {
      ...
      if (isHardcoded) {
        vulnerabilities.push({
          ...
          recommendation:
            'Avoid hardcoding credentials in source code. Use environment variables, secure credential stores, or configuration files that are not checked into version control.',
          code: `${variable.declarationType} ${variable.name} = ${
  ...
  private checkUnsafeFunctions(
    ...
    for (const ref of variable.references) {
      ...
      // Check for unsafe function usage
      const unsafeFunction = SecurityAnalyzer.UNSAFE_FUNCTIONS.find(pattern =>
        pattern.test(refLine)
      );
      ...
      if (unsafeFunction) {
        ...
        vulnerabilities.push({
          ...
          recommendation:
            'Avoid using eval(), dynamic Function creation, or setting HTML content with user-provided data. Use safer alternatives like JSON.parse() or DOM manipulation methods.',
          code: refLine.trim(),
          cwe,
        });
      }
  ...
  private checkCommandInjection(
    ...
    for (const ref of variable.references) {
      ...
      const commandInjectionPattern =
        SecurityAnalyzer.COMMAND_INJECTION_PATTERNS.find(pattern =>
          pattern.test(refLine)
        );
      ...
      if (commandInjectionPattern && refLine.includes(variable.name)) {
        vulnerabilities.push({
          ...
          recommendation:
            'Avoid passing user-provided data to system commands. If necessary, validate and sanitize input rigorously, use allowlists for permitted values, and avoid shell execution when possible.',
          code: refLine.trim(),
          cwe: 'CWE-78',
        });
      }
  ...
  private checkSqlInjection(
    ...
    for (const ref of variable.references) {
      ...
      const sqlInjectionPattern = SecurityAnalyzer.SQL_INJECTION_PATTERNS.find(
        pattern => pattern.test(refLine)
      );
      ...
      if (
        ...
        vulnerabilities.push({
          ...
          recommendation:
            'Use parameterized queries or prepared statements instead of string concatenation. Never trust user input when constructing SQL queries.',
          code: refLine.trim(),
          cwe: 'CWE-89',
        });
      }
  ...
  private checkPrototypePollution(
    ...
    if (variable.type === 'object') {
      for (const ref of variable.references) {
        ...
        if (
          (refLine.includes('Object.assign') ||
          ...
          vulnerabilities.push({
            ...
            recommendation:
              'Avoid modifying object prototypes with user input. Use Object.create(null) to create objects without prototypes, or use Map/Set instead of plain objects for user-controllable keys.',
            code: refLine.trim(),
            cwe: 'CWE-915',
          });
        }
   ...
   */
  private getLineAtPosition(content: string, lineNumber: number): string {
  ...
  public generateSecurityReport(
    ...
    const vulnerabilityRows = vulnerabilities
      .map(vuln => {
        const severityColor =
          vuln.severity === SecurityVulnerabilitySeverity.CRITICAL
            ? '#d93737'
        ...
        return `
        <tr>
          <td><span class="severity-badge" style="background-color: ${severityColor}">${
          vuln.severity
        }</span></td>
          <td>${vuln.type}</td>
          ...
          <td>
            <div class="code-block"><code>${this.escapeHtml(
              vuln.code
            )}</code></div>
            <div class="vuln-description">${vuln.description}</div>
            <div class="vuln-recommendation"><strong>Recommendation:</strong> ${
              vuln.recommendation
            }</div>
            ${
              vuln.cwe
                ? `<div class="vuln-cwe"><strong>CWE:</strong> <a href="https://cwe.mitre.org/data/definitions/${vuln.cwe.replace(
                    'CWE-',
                    ''
                  )}.html" target="_blank">${vuln.cwe}</a></div>`
                : ''
            }
          </td>
        </tr>
