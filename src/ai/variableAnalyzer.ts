...
export interface AIAnalysisResult {
  ...
  suggestions: string[];
  confidence: number;
  category: 'naming' | 'usage' | 'security' | 'performance' | 'general';
  explanation: string;
}

// Variable Quality Score Interface
export interface VariableQualityScore {
  name: string;
  namingScore: number;
  usageScore: number;
  scopeScore: number;
...
export class AIVariableAnalyzer {
  ...
  private static readonly NAMING_CONVENTIONS = {
  ...
  private static readonly COMMON_ISSUES = {
    tooShort: (name: string) => name.length < 2 && !/^[ijk]$/.test(name), // Allow i,j,k for loop counters
    tooLong: (name: string) => name.length > 30,
    meaningless: (name: string) =>
      /^(foo|bar|baz|x|y|z|a|b|c)$/.test(name) && !/^[ijk]$/.test(name),
    similar: (name: string, allNames: string[]) =>
      allNames.some(
        other =>
          other !== name &&
  ...
  private analyzeNamingConvention(
    ...
    if (AIVariableAnalyzer.COMMON_ISSUES.tooShort(name)) {
      ...
      suggestions.push(
        'Consider using a more meaningful name that describes its purpose'
      );
      explanation = 'Variable name lacks semantic meaning.';
    } else if (AIVariableAnalyzer.COMMON_ISSUES.similar(name, allNames)) {
      suggestions.push(
        'This variable name is similar to others in the same scope, which might cause confusion'
      );
      explanation =
    ...
    if (
      ...
      suggestions.push(
        `Consider prefixing boolean variables with 'is', 'has', 'can', etc.`
      );
      explanation =
      ...
      suggestions.push(
        'Consider using a plural form or suffix like "List" for array variables'
      );
      explanation =
        'Array variables typically use plural forms or suffixes like "List" or "Array".';
    } else if (
      variable.declarationType === 'const' &&
   ...
   */
  private analyzeUsagePattern(variable: VariableInfo): AIAnalysisResult | null {
    ...
    if (
      ...
      suggestions.push(
        'Consider using "const" since this variable is never reassigned'
      );
      explanation =
  ...
  private analyzeSecurityConcerns(
    ...
    if (
      sensitivePatterns.some(pattern => name.toLowerCase().includes(pattern))
    ) {
      const referenceContexts = references.map(ref => ref.context);

      // Check if sensitive data is logged or exposed
      if (
        referenceContexts.some(
          ctx =>
            ctx.includes('console.log') ||
            ctx.includes('alert(') ||
      ...
      if (
        referenceContexts.some(
          ctx => ctx.includes('localStorage') || ctx.includes('sessionStorage')
        )
      ) {
        suggestions.push(
          'Consider encrypting sensitive data before storing in browser storage'
        );
        explanation =
   ...
   */
  private analyzeScopeAndLifetime(
    variable: VariableInfo,
    allVariables: VariableInfo[]
  ): AIAnalysisResult | null {
    ...
    if (scope === 'global' && references.length > 5) {
      suggestions.push(
        'Consider reducing the scope of this variable or breaking it into smaller, more focused variables'
      );
      explanation =
    ...
    if (scope !== 'global' && references.length === 1) {
      suggestions.push(
        'This variable has limited usage. Consider inlining it or refactoring'
      );
      explanation =
    ...
    // Check for shadowing
    const shadowedVariables = allVariables.filter(
      v => v.name === name && v.filePath === variable.filePath && v !== variable
    );

    if (shadowedVariables.length > 0) {
      suggestions.push(
        'This variable shadows another variable with the same name in an outer scope'
      );
   ...
   */
  public calculateVariableQualityScore(
    variable: VariableInfo,
    allVariables: VariableInfo[]
    ...
    const overallScore = Math.round(
      namingScore * 0.4 + usageScore * 0.4 + scopeScore * 0.2
    );
