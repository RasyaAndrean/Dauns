...
export interface VariableDependency {
  ...
  dependencies: string[]; // Names of variables this variable depends on
  dependents: string[]; // Names of variables that depend on this variable
}

export interface DependencyGraph {
  [variableName: string]: VariableDependency;
}
...
export class DependencyAnalyzer {
      ...
      for (const ref of variableRefs) {
        ...
        for (const otherVariable of variables) {
          if (otherVariable.name !== variable.name) {
            ...
            if (
              this.isVariableInDeclaration(text, otherVariable, variable.name)
            ) {
              // Add dependency: otherVariable depends on variable
              if (
                !graph[otherVariable.name].dependencies.includes(variable.name)
              ) {
                graph[otherVariable.name].dependencies.push(variable.name);
              }
              if (
                !graph[variable.name].dependents.includes(otherVariable.name)
              ) {
   ...
   */
  private static isVariableInDeclaration(
    text: string,
    variable: VariableInfo,
    ...
    const varRegex = new RegExp(
      `\\b(${variable.kind})\\s+${this.escapeRegExp(variable.name)}\\b`
    );
   ...
   */
  static formatDependencyGraph(graph: DependencyGraph): string {
   ...
   */
  static findCircularDependencies(graph: DependencyGraph): string[][] {
    const circularDeps: string[][] = [];
    const visited: Set<string> = new Set();
    const recursionStack: string[] = [];

    const dfs = (variableName: string) => {
